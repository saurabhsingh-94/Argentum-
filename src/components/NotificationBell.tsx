  ...
  return (
    <>
      ...
      >
        ...
        />
        {unreadCount > 0 && (
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-green-500 rounded-full shadow-[0_0_10px_rgba(34,197,94,0.8)] border border-[#0a0a0a]" />
        )}
      </Link>
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            <div className="bg-[#111] border border-white/10 rounded-2xl p-4 shadow-2xl flex items-center gap-4 relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-r from-white/[0.02] to-transparent pointer-events-none" />
              
              <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-silver">
                {getIcon(toast.type)}
              </div>
              
              <div className="flex-1 flex flex-col gap-0.5">
                <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">New Notification</span>
                <p className="text-xs font-bold text-white leading-tight pr-4">
                  {toast.content}
                </p>
              </div>
              ...
              <button 
                ...
                className="p-1 hover:bg-white/10 rounded-lg transition-colors text-gray-600 hover:text-white"
              >
                <X size={14} />
              </button>
